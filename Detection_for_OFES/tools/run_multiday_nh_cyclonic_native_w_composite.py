...
DEFAULT_OUTPUT_ROOT = Path(
    ...
    r"\nh_open_ocean_cyclonic_native_w_pointwise_unrotated_eta_19910101_19910119"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--vertical-root", type=Path, default=DEFAULT_VERTICAL_ROOT,
                        help="Legacy umbrella root containing surface QC and the default vertical child.")
    parser.add_argument("--surface-qc-root", type=Path,
                        help="Explicit root containing daily_runs/YYYYMMDD/centers_hua_style.csv.")
    parser.add_argument("--vertical-run-root", type=Path,
                        help="Explicit vertical run root containing raw_detection/daily_runs/YYYYMMDD.")
    ...
    parser.add_argument(
        "--selection-mode", choices=["all_open_ocean", "strict_core_nh"],
        default="all_open_ocean",
    )
    parser.add_argument("--strict-core-diagnostics-root", type=Path)
    parser.add_argument(
...
def strict_core_scores(path: Path, minimum: float) -> pd.DataFrame:
    rows = pd.read_csv(path, low_memory=False)
    rows["section_bipolarity_min_fraction"] = rows[
        ["bipolar_fraction_0p6r", "bipolar_fraction_1p0r"]
    ].min(axis=1)
    return rows.loc[
        rows["section_bipolarity_min_fraction"].ge(minimum),
        [
            "hua_object_id", "accepted_below_surface_layers", "max_depth_m",
            "bipolar_fraction_0p6r", "bipolar_fraction_1p0r",
            "section_bipolarity_min_fraction",
        ],
    ].copy()
...
def main() -> None:
    ...
    diagnostics_root = args.strict_core_diagnostics_root or (
        args.vertical_root / "strict_core_diagnostics_local_lat_scaled"
    )
    surface_qc_root = args.surface_qc_root or (args.vertical_root / "surface_geometry_qc")
    vertical_run_root = args.vertical_run_root or (
        args.vertical_root / "vertical_continuation_local_step_2cells_section_bipolar"
    )
    ...
    for current in days:
        ...
        strict_scores = None
        if args.selection_mode == "strict_core_nh":
            if args.strict_core_catalog_root:
                catalog = (
                    args.strict_core_catalog_root / stem / "catalog"
                    / "vertical_continuation_object_catalog.csv"
                )
                ...
                diagnostic = (
                    diagnostics_root / stem / vertical_run_root.name / "section_bipolarity_object_summary.csv"
                )
                if not diagnostic.exists():
                    raise FileNotFoundError(f"Strict-core diagnostic is missing: {diagnostic}")
                strict_scores = strict_core_scores(diagnostic, args.strict_core_min_fraction)
        ...
        with ThreadPoolExecutor(max_workers=max(1, int(args.workers))) as pool:
            futures = [pool.submit(sample_object_geometry, raw, metas, obj, cargs) for obj in objects]
    ...
    manifest = {
        ...
        "source_vertical_root": str(args.vertical_root),
