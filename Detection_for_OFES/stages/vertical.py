...
from Detection_for_OFES.core.detection import (
    DetectionParams,
    EARTH_RADIUS_M,
    grid_spacing_km,
    hua_verify_radius,
    object_voxels_for_layer,
    refine_speed_min_subgrid,
    seeded_speed_min,
)
...
def params_from_args(args: argparse.Namespace) -> DetectionParams:
    ...
    return DetectionParams(
        ...
        deep_hua_mode=str(getattr(args, "deep_hua_mode", "tangent_then_near_closed_streamline")),
        boundary_mode=(
...
def select_deep_center(
    ...
    *,
    ...
    center_i, center_j, center_speed, steps = seeded_speed_min(speed, anchor_i, anchor_j, radius)
        ...
        for depth_index in range(1, max_layers):
            ...
            for state in active:
                ...
                if args.write_object_voxels:
                    voxels.extend(object_voxels_for_layer(
...
def main() -> None:
    ...
    parser.add_argument(
        "--deep-center-selection", choices=["local_step_section_bipolar"], default="local_step_section_bipolar",
        help="Canonical bounded local continuation with section-bipolar tie-breaking.",
    )
    parser.add_argument(
        "--deep-center-step-cells", type=int, default=2,
        help="Maximum per-layer displacement used by the canonical local center continuation.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--deep-tangent-tolerance-deg", type=float, default=45.0)
    parser.add_argument("--deep-min-tangent-fraction", type=float, default=0.35)
    ...
    parser.add_argument(
        "--deep-hua-mode", choices=["tangent_then_near_closed_streamline"], default="tangent_then_near_closed_streamline",
        help="Canonical tangent45/fraction0.35 gate followed by relaxed near-closed-streamline confirmation.",
    )
    parser.add_argument("--near-streamline-closure-tolerance-cells", type=float, default=2.50)
    parser.add_argument("--near-streamline-min-winding-turns", type=float, default=0.50)
    parser.add_argument("--near-streamline-min-points", type=int, default=12)
    parser.add_argument("--near-streamline-min-finite-fraction", type=float, default=0.90)
    ...
    parser.set_defaults(
        disable_angle_jump_hard_gate=True,
        disable_direction_exception_hard_gate=True,
        disable_opposite_reversal_hard_gate=True,
    )
