...
def write_parquet_parts_to_single(parts: list[str | Path], out_path: str | Path) -> None:
    ...
    if pq is None:
        frames = [pd.read_parquet(part, engine=DEFAULT_PARQUET_ENGINE) for part in parts if Path(part).exists()]
        if frames:
            pd.concat(frames, ignore_index=True).to_parquet(out_path, index=False, engine=DEFAULT_PARQUET_ENGINE)
        else:
            pd.DataFrame().to_parquet(out_path, index=False, engine=DEFAULT_PARQUET_ENGINE)
        return
...
def read_partitioned_table(root: str | Path) -> pd.DataFrame:
    ...
    return pd.read_parquet(root, engine=DEFAULT_PARQUET_ENGINE)
