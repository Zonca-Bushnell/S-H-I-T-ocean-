...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--launch-stagger-seconds", type=float, default=90.0)
...
def start(root: Path, spec: WorkerSpec, log_dir: Path, supervisor_log: Path) -> subprocess.Popen[bytes]:
    ...
    command = [
        sys.executable, "-u", "-m", "Detection_for_OFES.datasets.download_ofes2_monthly_prho",
        "--output-root", str(root), "--start-year", str(spec.start_year), "--end-year", str(spec.end_year),
        "--worker-name", spec.name, "--lat-block-rows", "5",
        "--block-retries", "0", "--retry-seconds", "15", "--retry-max-seconds", "300",
...
def main() -> None:
    args = parse_args()
    if args.stall_minutes <= 0 or args.poll_seconds <= 0 or args.launch_stagger_seconds < 0:
        raise ValueError("Supervisor timing parameters must be positive")
    ...
    while True:
        ...
        for spec in SPECS:
            ...
            if process is None or process.poll() is not None:
                ...
                next_launch_at = time.monotonic() + args.launch_stagger_seconds
                continue
    ...
    try:
        subprocess.run([sys.executable, "-u", "-m", "Detection_for_OFES.datasets.build_ofes2_prho_annual_mss", "--output-root", str(args.output_root)], stdout=build_log, stderr=subprocess.STDOUT, check=True)
    finally:
        build_log.close()
