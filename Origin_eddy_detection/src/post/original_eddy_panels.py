...
def _is_axis_curved_fields(fields: dict[str, dict[str, np.ndarray] | None]) -> bool:
    for part in fields.values():
        if not part:
            continue
        for key in ("upper_velocity_section", "lower_velocity_section", "w_section"):
            section = part.get(key)
            if section is not None and str(section.get("section_axis", "")).startswith("axis-following"):
                return True
    return False


def _axis_section_for_geometry(part: dict[str, np.ndarray] | None) -> dict[str, np.ndarray] | None:
    if not part:
        return None
    for key in ("upper_velocity_section", "lower_velocity_section", "w_section"):
        section = part.get(key)
        if section is not None and "centerline_x_km" in section:
            return section
    return None


def _plot_axis_curved_geometry_3d(
    ax,
    *,
    selected: SelectedObject,
    object_layers: pd.DataFrame,
    section: dict[str, np.ndarray] | None,
    jump_from_depth_index: int | None,
    jump_to_depth_index: int | None,
    title: str,
) -> None:
    if section is None or jump_to_depth_index is None:
        ax.text2D(0.08, 0.5, "axis-curved geometry unavailable", transform=ax.transAxes, fontsize=9)
        ax.set_axis_off()
        return

    centers = object_layers.sort_values("depth_index").copy()
    cx = centers["delta_x_km"].to_numpy(dtype="f8")
    cy = centers["delta_y_km"].to_numpy(dtype="f8")
    cz = centers["depth_m"].to_numpy(dtype="f8")
    ck = centers["depth_index"].to_numpy(dtype="i8")

    line_x = np.asarray(section.get("centerline_x_km", cx), dtype="f8")
    line_y = np.asarray(section.get("centerline_y_km", cy), dtype="f8")
    line_z = np.asarray(section.get("centerline_depth_m", cz), dtype="f8")
    nx = np.asarray(section.get("axis_normal_x", np.zeros_like(line_z)), dtype="f8")
    ny = np.asarray(section.get("axis_normal_y", np.zeros_like(line_z)), dtype="f8")
    tx = np.asarray(section.get("axis_tangent_x", np.zeros_like(line_z)), dtype="f8")
    ty = np.asarray(section.get("axis_tangent_y", np.zeros_like(line_z)), dtype="f8")

    valid_line = np.isfinite(line_x) & np.isfinite(line_y) & np.isfinite(line_z)
    if np.any(valid_line):
        ax.plot(line_x[valid_line], line_y[valid_line], line_z[valid_line], color="0.15", lw=1.8, label="interpolated axis")
    ax.scatter(cx, cy, cz, s=12, c=cz, cmap="viridis", depthshade=False, label="layer centers")

    def layer_point(depth_index: int | None) -> tuple[float, float, float] | None:
        if depth_index is None:
            return None
        mask = ck == int(depth_index)
        if not np.any(mask):
            return None
        i = int(np.where(mask)[0][0])
        return float(cx[i]), float(cy[i]), float(cz[i])

    p0 = layer_point(jump_from_depth_index)
    p1 = layer_point(jump_to_depth_index)
    if p0 is not None:
        ax.scatter([p0[0]], [p0[1]], [p0[2]], marker="+", s=90, c="red", linewidths=2.0, label="upper/from")
    if p1 is not None:
        ax.scatter([p1[0]], [p1[1]], [p1[2]], marker="x", s=80, c="deepskyblue", linewidths=2.0, label="lower/to")
    if p0 is not None and p1 is not None:
        ax.plot([p0[0], p1[0]], [p0[1], p1[1]], [p0[2], p1[2]], color="red", lw=2.0, alpha=0.8, label="jump")

    normal_half = max(float(selected.radius_m) / 1000.0, 45.0)
    for depth_index, color in ((jump_from_depth_index, "red"), (jump_to_depth_index, "deepskyblue")):
        if depth_index is None or line_z.size == 0:
            continue
        iz = int(np.nanargmin(np.abs(line_z - float(layer_point(depth_index)[2] if layer_point(depth_index) else 0.0))))
        if not np.isfinite(line_x[iz]) or not np.isfinite(line_y[iz]):
            continue
        ax.plot(
            [line_x[iz] - normal_half * nx[iz], line_x[iz] + normal_half * nx[iz]],
            [line_y[iz] - normal_half * ny[iz], line_y[iz] + normal_half * ny[iz]],
            [line_z[iz], line_z[iz]],
            color=color,
            lw=1.6,
            alpha=0.9,
        ...
        ax.quiver(
            ...
            line_y[iz],
            line_z[iz],
            tx[iz] * normal_half * 0.45,
            ty[iz] * normal_half * 0.45,
            0,
            color="0.25",
    ...
    ax.invert_zaxis()
    ax.set_xlabel("x offset (km)", labelpad=2)
    ax.set_ylabel("y offset (km)", labelpad=2)
...
def _plot_9panel(
    ...
    show_axis_geometry = _is_axis_curved_fields(fields)
    fig = plt.figure(figsize=(38 if show_axis_geometry else 32, 18), constrained_layout=True)
    gs = fig.add_gridspec(
        5,
        8 if show_axis_geometry else 6,
        height_ratios=[1.0, 1.0, 1.0, 1.0, 0.85],
        width_ratios=(
            [0.9, 0.9, 1.05, 1.05, 1.05, 1.05, 0.95, 0.95]
            if show_axis_geometry
            else [0.9, 0.9, 1.05, 1.05, 1.05, 1.05]
        ),
    )
    ...
    ax12 = fig.add_subplot(gs[0:2, 6:8], projection="3d") if show_axis_geometry else None
    ax13 = fig.add_subplot(gs[2:4, 6:8], projection="3d") if show_axis_geometry else None
    ...
    if show_axis_geometry and ax12 is not None:
        _plot_axis_curved_geometry_3d(
            ax12,
            selected=selected,
            object_layers=offsets,
            section=_axis_section_for_geometry(first_fields),
            jump_from_depth_index=selected.jump_from_depth_index,
            jump_to_depth_index=selected.jump_to_depth_index,
            title="12  J1 axis-curved geometry",
        )
    ...
    if show_axis_geometry and ax13 is not None:
        _plot_axis_curved_geometry_3d(
            ax13,
            selected=selected,
            object_layers=offsets,
            section=_axis_section_for_geometry(second_fields),
            jump_from_depth_index=selected.second_jump_from_depth_index,
            jump_to_depth_index=selected.second_jump_to_depth_index,
            title="13  J2 axis-curved geometry",
        )
...
def _make_vertical_w_section(
    *,
    ...
    if section_mode == "axis_curved":
        result.update(
            {
                "centerline_x_km": np.asarray(center_x_full, dtype="f8"),
                "centerline_y_km": np.asarray(center_y_full, dtype="f8"),
                "centerline_depth_m": np.asarray(depth, dtype="f8"),
                "axis_tangent_x": np.asarray(ny_axis, dtype="f8"),
                "axis_tangent_y": np.asarray(-nx_axis, dtype="f8"),
                "axis_normal_x": np.asarray(nx_axis, dtype="f8"),
                "axis_normal_y": np.asarray(ny_axis, dtype="f8"),
            }
        )
    return result
        ...
        result.update(
            {
                ...
                "centerline_y_km": np.asarray(center_y_full, dtype="f8"),
                "centerline_depth_m": np.asarray(depth, dtype="f8"),
                "axis_tangent_x": np.asarray(tx, dtype="f8"),
                "axis_tangent_y": np.asarray(ty, dtype="f8"),
                "axis_normal_x": np.asarray(nx_axis, dtype="f8"),
                "axis_normal_y": np.asarray(ny_axis, dtype="f8"),
            }
        )
