...
def _read_table(path: Path) -> pd.DataFrame:
    if path.with_suffix(".parquet").exists():
        return pd.read_parquet(path.with_suffix(".parquet"), engine=DEFAULT_PARQUET_ENGINE)
...
def run(args: argparse.Namespace) -> None:
    ...
    event_df.to_parquet(out_dir / "feature_track_events.parquet", index=False, engine=DEFAULT_PARQUET_ENGINE)
    event_df.to_csv(out_dir / "feature_track_events.csv", index=False)
    overlap_df.to_parquet(out_dir / "feature_overlap_table.parquet", index=False, engine=DEFAULT_PARQUET_ENGINE)
    overlap_df.to_csv(out_dir / "feature_overlap_table.csv", index=False)
    groups.to_parquet(out_dir / "group_tracks.parquet", index=False, engine=DEFAULT_PARQUET_ENGINE)
    groups.to_csv(out_dir / "group_tracks.csv", index=False)
    tracks.to_parquet(out_dir / "feature_tracks.parquet", index=False, engine=DEFAULT_PARQUET_ENGINE)
    tracks.to_csv(out_dir / "feature_tracks.csv", index=False)
    points.to_parquet(out_dir / "feature_track_points.parquet", index=False, engine=DEFAULT_PARQUET_ENGINE)
...
def main() -> None:
    ...
    parser.add_argument("--skip-plots", action="store_true", help="Skip matplotlib tracking figures and write tables only.")
