...
DEFAULT_OUTPUT_ROOT = Path(r"E:\DATA\01_Eddy_correspond\03_Original_detection")
DEFAULT_DATA_ROOT = Path(r"F:\Global Ocean Ensemble Physics Reanalysis Kuroshio Current")
DEFAULT_FILTER_ROOT = DEFAULT_DATA_ROOT / "FILTER"
...
ORIGIN_DETECTION_TO_SHAPE_MOUTHFUL = (
    "hua_b3_start2 + 30-180d bandpass + boundary_monotonic + "
    "strict_contiguous + local_1_24deg_refined_velocity_center + life30 shape"
)
...
def _month_shards(start: str, end: str) -> list[tuple[str, str, str]]:
    start_dt = _parse_date(start)
    end_dt = _parse_date(end)
    shards: list[tuple[str, str, str]] = []
    current = start_dt
    while current <= end_dt:
        ...
        shard_end = min(end_dt, next_month - timedelta(days=1))
        tag = f"{current.year}{current.month:02d}"
        shards.append((_date_label(current), _date_label(shard_end), tag))
        current = shard_end + timedelta(days=1)
    return shards


def _year_shards(start: str, end: str) -> list[tuple[str, str, str]]:
    start_dt = _parse_date(start)
    end_dt = _parse_date(end)
    shards: list[tuple[str, str, str]] = []
    current = start_dt
    while current <= end_dt:
        shard_end = min(end_dt, datetime(current.year, 12, 31))
        tag = f"{current.year}"
        shards.append((_date_label(current), _date_label(shard_end), tag))
        current = shard_end + timedelta(days=1)
    return shards
...
def _detection_shards(start: str, end: str, mode: str) -> list[tuple[str, str, str]]:
    ...
    if mode == "month":
        return _month_shards(start, end)
    if mode == "quarter":
        return _quarter_shards(start, end)
    raise ValueError(f"Unsupported detection shard mode: {mode}")
...
def run_matlab_candidate_precompute(args: argparse.Namespace, paths: Paths) -> None:
    cache_dir = Path(args.candidate_cache_dir) if args.candidate_cache_dir else paths.output_root / "matlab_candidate_cache"
    matlab_dir = Path(__file__).resolve().parents[2] / "matlab"
    cmd_text = (
        f"addpath({_matlab_quote(matlab_dir)}); "
        "origin_precompute_candidates("
        f"{_matlab_quote(args.filter_root)}, "
        f"{_matlab_quote(cache_dir)}, "
        f"{_matlab_quote(args.start)}, "
        f"{_matlab_quote(args.end)}, "
        f"{int(args.ssh_window_cells)}, "
        f"{int(args.max_candidates_per_day)}, "
        f"{str(bool(args.matlab_use_gpu)).lower()});"
    )
    _run(
        [str(args.matlab_exe), "-batch", cmd_text],
        paths.logs_dir / "matlab_candidate_precompute.log",
    )
    args.candidate_cache_dir = cache_dir
...
def _table_count(path: Path) -> dict[str, object]:
    ...
    try:
        ...
        df = pd.read_parquet(path, engine=DEFAULT_PARQUET_ENGINE)
...
def write_runtime_config(args: argparse.Namespace, paths: Paths) -> None:
    ...
    if DRY_RUN:
        ...
        print(f"[dry-run] production science: {ORIGIN_DETECTION_TO_SHAPE_MOUTHFUL}", flush=True)
        return
    ...
    try:
        ...
        paths.config_path.write_text(json.dumps(cfg, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    else:
...
def detection_command(args: argparse.Namespace, paths: Paths, start: str, end: str, extra: list[str]) -> list[str]:
    ...
    cmd = [
        ...
        str(args.netcdf_chunk_cache_mb),
        "--preload-day-uv",
        "--write-object-voxels",
        "--resume",
        "--skip-axis-examples",
...
def run_pipeline(args: argparse.Namespace, paths: Paths) -> None:
    ...
    if "representative" in args.stages:
        raise SystemExit(
            "Origin_eddy_detection stops at final shape. "
            "Representative-vortex and transport stages remain in Zhe and are not copied here."
        )
    if not DRY_RUN:
    ...
    if bool(args.matlab_candidate_precompute):
        if not DRY_RUN:
            paths.logs_dir.mkdir(parents=True, exist_ok=True)
        run_matlab_candidate_precompute(args, paths)
    if "detect" in args.stages:
        shard_cmds = [
            (detection_command(args, paths, start, end, ["--partial-only"]), paths.logs_dir / f"detect_{tag}.log")
            for start, end, tag in _detection_shards(args.start, args.end, str(args.detect_shard_mode))
        ]
...
def write_manifests(args: argparse.Namespace, paths: Paths) -> None:
    ...
    result_manifest = {
        ...
        "authority": f"{args.region_name} Hua/Nencioli b3_start2 life{int(args.lifetime_min_days)} detection-to-shape",
        ...
        "tables": {
            ...
            "shape_tracks": _table_count(paths.output_root / paths.shape_name / "shape_tracks.parquet"),
        },
    }
...
def _manifest_markdown(manifest: dict[str, object]) -> str:
    ...
    lines.extend(["", "## Notes", "", f"- Detection-to-shape mouthful: `{ORIGIN_DETECTION_TO_SHAPE_MOUTHFUL}`."])
    return "\n".join(lines) + "\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--representative-relative-dir", type=Path, default=Path("result_coherent_only") / "representative_vortex_me_liutex", help=argparse.SUPPRESS)
    parser.add_argument("--radial-seed-relative-dir", type=Path, default=Path("result_coherent_only") / "representative_vortex_radial_seed", help=argparse.SUPPRESS)
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Per-variable NetCDF chunk cache passed to detection workers.",
    )
    parser.add_argument("--candidate-cache-dir", type=Path, default=None)
    parser.add_argument(
        "--matlab-candidate-precompute",
        action="store_true",
        help="Use the MATLAB backend under matlab/ to precompute surface SSH candidate caches before Python Hua checks.",
    )
    ...
    parser.add_argument("--representative-workers", type=int, default=1, help=argparse.SUPPRESS)
    parser.add_argument("--representative-chunk-days", type=int, default=7, help=argparse.SUPPRESS)
    parser.add_argument("--orientation", choices=["turned", "unturned", "both"], default="turned", help=argparse.SUPPRESS)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Comma-separated stages for this extraction: detect,tracking,catalog_shape.",
    )
