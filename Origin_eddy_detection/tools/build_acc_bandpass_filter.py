...
FILL_LIMIT = 1.0e10
DEFAULT_INPUT_ROOT = Path(r"F:\Global Ocean Ensemble Physics Reanalysis  ACC")
DEFAULT_OUTPUT_DIR = Path(r"E:\DATA\01_Eddy_correspond\03_Original_detection\ACC\Filter")
DEFAULT_TEMP_DIR = Path(r"E:\DATA\01_Eddy_correspond\03_Original_detection\ACC\_filter_work")
...
def variable_complete(output_paths: dict[int, Path], variable: str) -> bool:
    for path in output_paths.values():
        if not path.exists():
            return False
        try:
            with Dataset(path) as ds:
                if variable not in ds.variables:
                    return False
                if int(getattr(ds.variables[variable], "bandpass_complete", 0)) != 1:
                    return False
        except Exception:
            return False
    return True
...
def write_manifest(
    ...
    manifest = {
        ...
        "temp_dir": str(Path(args.temp_dir)),
        "variables": list(variables),
        "start": str(args.start),
        "end": str(args.end),
        "period_low_days": float(args.period_low_days),
        "period_high_days": float(args.period_high_days),
        "filter_backend": "scipy.signal.butter(order=4, output='sos') + sosfiltfilt",
        "cache_mode": str(args.cache_mode),
        "workers": int(args.workers),
        "depth_block": int(args.depth_block),
        "lat_block": int(args.lat_block),
        "lon_block": int(args.lon_block),
        "files": [],
    }
    for year, path in sorted(output_paths.items()):
        info = {
            "year": int(year),
            "path": str(path),
        ...
        if path.exists():
            with Dataset(path) as ds:
                for variable in variables:
                    complete = variable in ds.variables and int(getattr(ds.variables[variable], "bandpass_complete", 0)) == 1
                    info["variables_complete"][variable] = bool(complete)
        manifest["files"].append(info)
    (output_dir / "bandpass_filter_manifest.json").write_text(
...
def main() -> None:
    ...
    if args.cache_mode == "memmap":
        for variable in variables:
            if not args.force and variable_complete(output_paths, variable):
                print(f"[bandpass] skip complete variable {variable}", flush=True)
                continue
            ...
            try:
                ...
                write_manifest(output_dir, output_paths, variables, args, started_at)
            finally:
