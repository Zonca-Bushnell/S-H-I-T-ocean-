...
for k = 45:length(list_time_unique)
    ...
    parfor j = 1:size(PSAL,2)
        ...
        for i = 1:ilen
            ...
            if sum(isnan(SA_in)) < 85
                Den(i,j,:) = gsw_rho(SA_in,CT_in,pres);
            end
        end
    end
    ...
    filename = ['/Users/Root/Data/Argo_Data/Self_ISAS_Argo_Density' '/Den_' ...
...
% [xv,yv,zv] = meshgrid(lat_in,lon_in,Depth1);
% TEMP_interp = interp3(x,y,z,TEMP,xv,yv,zv);

%%
data_inuse = Den_interp(:,:,2);
...
hold on
p = m_pcolor(lon_interp,lat_interp,data_inuse');
p.EdgeColor = 'none';
p.FaceColor = 'interp'; 
