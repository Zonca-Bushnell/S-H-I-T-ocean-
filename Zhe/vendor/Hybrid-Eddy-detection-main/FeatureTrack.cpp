   ...
   */
    ...
    for (int obj1 = 0; obj1<numObjs; obj1++){
        ...
        for (int i=0; i<NumOvlp; i++){
            //               GenCombination(Comb, i);
            float denom = GeomMean3(i, Overlaps, obj1, direct,t1,t2);
            if(denom <= 0)
                continue;
            float cost(Intersect2(i, Overlaps, obj1, direct,OverlapTable)/denom);
            int Score((int)(1000*cost));
...
/* ScoreBoard looks like OverlapTable. Each element in this 2d array is the score of two objs.
   ...
   */
    ...
    for (int obj1 = 0; obj1<numObjs; obj1++){
        ...
        for (int i=0; i<NumOvlp; i++){
            //               GenCombination(Comb, i);
            float denom = GeomMean3_2D(i, Overlaps, obj1, direct,t1,t2);
            if(denom <= 0)
                continue;
            float cost(Intersect2(i, Overlaps, obj1, direct,OverlapTable)/denom);
            int Score((int)(1000*cost));
