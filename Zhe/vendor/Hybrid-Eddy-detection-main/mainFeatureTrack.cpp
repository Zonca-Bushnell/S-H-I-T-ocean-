...
double bounds[6];
bool surrogateTrackingMode = false;
                     ...
                     int &TimePrecision, int &TimeIncrement, float &thresh1, float &thresh2,int & x_dim, int &y_dim, int &z_dim,int & x0_dim, int &y0_dim, int &z0_dim,  int & x1_dim, int &y1_dim, int &z1_dim, int& CircleStartRadius, dataVariableName& datavariableName)
{
    ...
    smallestObjVolToTrack, variableNames, xdim, ydim, zdim, x0dim, y0dim, z0dim, x1dim, y1dim, z1dim, FileGeneratedTrackFileName,SearchRadius, surrogateTracking;
    //vector<string> variableNamesvect;
    ...
    surrogateTracking = string("SURROGATE_TRACKING:").c_str();
    ...
    if (myreadfile.is_open())
    {
        ...
        while (!myreadfile.eof())
        {
            ...
            if ( output == surrogateTracking)
            {
                myreadfile >> output;
                surrogateTrackingMode = (output == "1" || output == "true" || output == "TRUE" || output == "True");
            }
    ...
    cout<<" surrogateTrackingMode: "<< surrogateTrackingMode <<endl;
...
//    float *node_data = NULL;
    ...
    if(surrogateTrackingMode)
    {
        ...
        if (strcmp (FileBaseName.c_str(),nonamecharacter.c_str()) == 0 )
            CreateListFile(base_GeneratedTrackFileNameOriginal,datapath,listfile,FileBaseName,InitialtimeStep,FinaltimeStep,TimeIncrement,TimePrecision );
        else
            CreateListFile(base_GeneratedTrackFileNameOriginal, datapath+FileBaseName, listfile,FileBaseName,InitialtimeStep, FinaltimeStep, TimeIncrement, TimePrecision );
        ...
        for (int currentTime = 0; currentTime <= maxCycleNumber; currentTime++ )
        {
            ...
            string base_GeneratedTrackFileName = base_GeneratedTrackFileNameOriginal;
            string mypolyfile;
            if (strcmp (FileBaseName.c_str(),nonamecharacter.c_str()) == 0 )
            {
                mypolyfile = base_GeneratedTrackFileName + currenttimevalue + ".poly";
            }
            else
            {
                mypolyfile = base_GeneratedTrackFileName + FileBaseName + currenttimevalue + ".poly";
                base_GeneratedTrackFileName = base_GeneratedTrackFileName + FileBaseName;
            }

            char mypolyfile1[Consts::MAXLEN];
            sprintf(mypolyfile1, "%s", mypolyfile.c_str());
            int finished = 0;
            string TrackingMode = "3D";
            cout << "SURROGATE_TRACKING frame " << currenttimevalue << " using " << mypolyfile << endl;
            BeginFeatureTrack(base_GeneratedTrackFileName, currenttimevalue, mypolyfile1, finished,
                              deltaxval, deltayval, deltazval, listfile, InitialtimeStep,
                              TimeIncrement, currentTime, TimePrecision, TrackingMode);
        }
        return 0;
    }
