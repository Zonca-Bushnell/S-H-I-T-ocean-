...
from .run_streaming_layer_identification import (
    ...
    is_year_shared_cache_enabled,
    is_year_ram_cache_enabled,
...
def _catalog_part_path(root: Path, day, name: str) -> Path:
    return root / f"{name}_parts" / f"year={day:%Y}" / f"{name}_{day:%Y%m%d}.parquet"


def _day_catalog_parts_exist(root: Path, day) -> bool:
    return table_exists(_catalog_part_path(root, day, "layer_observations")) and table_exists(_catalog_part_path(root, day, "vertical_objects"))


def _next_object_id_after_part(root: Path, day) -> int | None:
    path = _catalog_part_path(root, day, "vertical_objects")
    if not path.exists():
        return None
    ids = pd.read_parquet(path, columns=["eddy3d_object_id"])
    if ids.empty:
        return None
    return int(ids["eddy3d_object_id"].max()) + 1
...
def _build_streaming_catalog(
    ...
    *,
    ...
    layer_rows = 0
    object_rows = 0
    mode = cache_mode(config)
    use_year_cache = is_year_ram_cache_enabled(config)
    use_year_shared = is_year_shared_cache_enabled(config)
    use_day_preload = mode == "day_preload"
    desc = f"Streaming detect + vertical link ({workers} workers/day"
    if use_year_shared:
        desc += ", year shared cache)"
    elif use_year_cache:
        desc += ", year RAM cache)"
    elif use_day_preload:
        desc += ", day preload)"
    else:
        desc += ")"
    with tqdm(total=len(days), desc=desc, unit="day") as bar:
        if use_year_shared:
            days_by_year: dict[int, list] = {}
            for day in days:
                days_by_year.setdefault(day.year, []).append(day)
            for year in sorted(days_by_year):
                year_days = days_by_year[year]
                owner = create_shared_year_cache(config, source_index[year_days[0]])
                try:
                    with ProcessPoolExecutor(
                        ...
                        for day in year_days:
                            if not force and _day_catalog_parts_exist(catalog_dir, day):
                                layer_parts.append(_catalog_part_path(catalog_dir, day, "layer_observations"))
                                object_parts.append(_catalog_part_path(catalog_dir, day, "vertical_objects"))
                                skipped_next_id = _next_object_id_after_part(catalog_dir, day)
                                if skipped_next_id is not None:
                                    next_object_id = max(next_object_id, skipped_next_id)
                                bar.update(1)
                                bar.set_postfix(date=day.isoformat(), skipped="catalog_part")
                                continue
                            obs, contours = identify_day_year_shared(
                                config,
                                source_index[day],
                                depth_indices,
                                workers=workers,
                            ...
                            bar.update(1)
                            bar.set_postfix(date=day.isoformat(), detections=len(obs), objects=len(objects_df), year_shared=year)
                finally:
                    owner.cleanup()
        elif use_year_cache:
            days_by_year: dict[int, list] = {}
            ...
            try:
                for year in sorted(days_by_year):
                    year_days = days_by_year[year]
                    cache = load_source_year_cache(config, source_index[year_days[0]])
                    ...
                    with ProcessPoolExecutor(max_workers=int(workers)) as executor:
                        for day in year_days:
                            if not force and _day_catalog_parts_exist(catalog_dir, day):
                                ...
                                bar.set_postfix(date=day.isoformat(), skipped="catalog_part")
                                continue
                            obs, contours = identify_day_year_cached(
                                config,
                                source_index[day],
                                depth_indices,
                                workers=workers,
                            ...
                            if write_layer_checkpoints:
                                _write_layer_checkpoint(config, day, obs, contours)
                            bar.update(1)
                            bar.set_postfix(date=day.isoformat(), detections=len(obs), objects=len(objects_df), year_cache=year)
            finally:
                set_year_cache(None)
        else:
            for day in days:
                if not force and _day_catalog_parts_exist(catalog_dir, day):
                    ...
                    continue
                if use_day_preload:
                    obs, contours = identify_day_preloaded(
                        config,
                        source_index[day],
                        depth_indices,
                        workers=workers,
