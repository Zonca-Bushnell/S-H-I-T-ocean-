...
DEFAULT_OUTPUT_DIR = Path(r"/root/autodl-fs/2020_2022_acc")
DEFAULT_TEMP_DIR = Path(r"/root/autodl-tmp/acc_subset_tmp")

GLOBAL_START = date(2020, 1, 1)
GLOBAL_END = date(2023, 1, 1)
SAMPLE_START = date(2020, 1, 1)
SAMPLE_END = date(2020, 1, 3)
...
def subset_temp_name(start: date, end: date) -> str:
    return f"acc_subset_{start:%Y%m%d}_{end:%Y%m%d}.nc"
...
def create_part_from_subset(
    ...
    *,
    ...
    with Dataset(tmp_path, "w", format="NETCDF4") as dst:
        ...
        dst.title = "ACC Copernicus Marine server-side subset, streamed into annual file"
    ...
    *,
    ...
    with Dataset(subset_path) as src:
        ...
        with Dataset(tmp_path, "w", format="NETCDF4") as dst:
            for attr in src.ncattrs():
                dst.setncattr(attr, src.getncattr(attr))
            dst.title = "ACC Copernicus Marine server-side annual subset"
            dst.source_dataset_id = DATASET_ID
            dst.source_dataset_version = DATASET_VERSION
...
def process_window(
    ...
    *,
    ...
    with Dataset(part_path, "r+") as dst:
        ...
        dst.history = (
            f"Completed by download_acc_subset_streaming.py on "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Copernicus Marine ACC server-side subsets in low-memory time blocks."
    )
