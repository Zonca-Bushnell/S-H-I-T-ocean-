...
DEFAULT_OUTPUT_DIR = Path("/root/autodl-fs/kuroshiou/raw")
DEFAULT_TEMP_DIR = Path("/root/autodl-tmp/kuroshiou_work/download_tmp")
...
def create_part_from_subset(
    ...
    *,
    ...
    with Dataset(tmp_path, "w", format="NETCDF4") as dst:
        ...
        dst.title = "Kuroshiou/Kuroshio Copernicus Marine server-side subset, streamed into annual file"
    ...
    *,
    ...
    with Dataset(subset_path) as src:
        ...
        with Dataset(tmp_path, "w", format="NETCDF4") as dst:
            for attr in src.ncattrs():
                dst.setncattr(attr, src.getncattr(attr))
            dst.title = "Kuroshiou/Kuroshio Copernicus Marine server-side annual subset"
            dst.source_dataset_id = DATASET_ID
            dst.source_dataset_version = DATASET_VERSION
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download Copernicus Marine Kuroshiou/Kuroshio server-side subsets in low-memory time blocks."
    )
