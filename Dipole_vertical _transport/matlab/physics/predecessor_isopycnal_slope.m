function [dzdx_stack, dzdy_stack] = predecessor_isopycnal_slope(rho_stack, depth_levels, dx_m, dy_m, upward_coordinate)
    ...
    if use_parallel
        parfor ii = 1:ny
            [dzdx_row, dzdy_row] = predecessor_isopycnal_slope_row(ii, rho_clean, z_axis, dx_m, dy_m);
...
function [dzdx_row, dzdy_row] = predecessor_isopycnal_slope_row(ii, rho_clean, z_axis, dx_m, dy_m)
    [ny, nx, nz] = size(rho_clean);
