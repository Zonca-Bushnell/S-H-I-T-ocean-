function write_method_doc(path, argo_mat, history_argo_mat, meta_dir, boa_pden_root, output_root, bbox, crossing_lats, target_lat, intersect_radius_r, match_mode, boa_background_mode, time_window_days, core_min_m, core_max_m, density_variable, z_rho_min_m, z_rho_max_m, min_drho_dz, max_rho_bracket_dz_m)
    ...
    fprintf(fid, '- 等密面几何：正式生产结果固定为 `BOA monthly climatology -> D''_rho -> Cressman composite -> isopycnal geometry`。\n');
    fprintf(fid, '- BOA 背景模式：`%s`。默认按月份平均 `PDen1000_YYYYMM.mat`，对每个 profile 的 `lon/lat/month` 双线性插值得到背景密度剖面。\n', boa_background_mode);
    fprintf(fid, '- `D_rho` 反插值：profile 和 BOA 背景均只使用显式 bracket crossing，不再用全剖面 fallback；有效窗口 `%.0f-%.0f m`，`abs(local_drho_dz) >= %.3g`，bracket 厚度 `<= %.0f m`。\n', z_rho_min_m, z_rho_max_m, min_drho_dz, max_rho_bracket_dz_m);
    fprintf(fid, '- 禁止用 `eta_rho ≈ rho''/(dρ/dD)` 这类密度导数反推等密面位移；密度导数只允许用于热成风速度剪切等物理计算。\n');
    fprintf(fid, '- 计算边界：composite analysis 只输出等密面几何、速度、支撑和 QC；`term1/term2/rebuild_W` 由独立 physics rebuild 模块计算。\n');
    ...
    fprintf(fid, '- 深度变量约定：兼容字段 `z_rho_m`、`z_rho_bg_m`、`z_rho_anom_m` 的物理含义均为 `D_rho`，即正深度向下，便于海洋剖面阅读。\n');
    ...
    fprintf(fid, '- `c_x_raw` 来自 META track 相邻点中央差分；`u_bg` 为同 crossing 组、同极性、匹配 Core Argo 的 parking drift 纬向均值；`c_x_rel = mean(c_x_raw) - mean(u_bg)`。主图采用 `term1 = +c_x_rel dD''_rho/dx`，`term2 = -[(u_pk-c_x_raw, v_pk) · grad(D''_rho)]`，`rebuild_W = term1 + term2`。\n');
