function write_group_doc_3d(path, matches, grid3d, polarity, band_label)
    ...
    if strcmp(grid3d.vertical_mode, 'thermal_wind_depth_stack')
        ...
        fprintf(fid, '- 公式：`term1 = +c_x_rel dD''_rho/dx`，`term2 = -[(u_tw-u_bg,v_tw)·grad(D''_rho)]`，`W_up = term1 + term2`。\n');
    else
        fprintf(fid, '- 公式：`term1 = +c_x_rel dD''_rho/dx`，`term2 = -[(u_pk-u_bg,v_pk)·grad(D''_rho)]`，`W_up = term1 + term2`。\n');
    end
    fprintf(fid, '- W 符号：向上为正；深度和 `D''_rho` 按正深度向下保存和标注。历史字段名 `z_rho_anom` 仅为兼容名。\n');
