function write_group_doc_3d(path, matches, grid3d, polarity, band_label)
    ...
    if strcmp(grid3d.vertical_mode, 'thermal_wind_depth_stack')
        ...
        fprintf(fid, '- 公式：`term1 = +c_x_rel dD''_rho/dx`，`term2 = -[(u_tw-c_x_raw,v_tw)·grad(D''_rho)]`，`W = term1 + term2`。\n');
    else
        fprintf(fid, '- 公式：`term1 = +c_x_rel dD''_rho/dx`，`term2 = -[(u_pk-c_x_raw,v_pk)·grad(D''_rho)]`，`W = term1 + term2`。\n');
    end
    fprintf(fid, '- W 符号：向上为正；深度和 `z_rho_anom` 按正深度向下保存和标注。\n');
    fprintf(fid, '- 计算边界：3D composite 阶段只生成等密面几何、密度场、热成风速度和支撑；`term1/term2/W` 由 `physics/rebuild_w_3d_from_geometry.m` 独立生成。\n');
