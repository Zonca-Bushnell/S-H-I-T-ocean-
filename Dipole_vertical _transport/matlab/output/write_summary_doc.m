function write_summary_doc(path, summary_rows, output_root)
    ...
    fprintf(fid, '- 输出根目录：`%s`\n', output_root);
    fprintf(fid, '- 主图变量：W 向上为正，正式几何统一为 `D''_rho` 正深度向下，`term1 = +c_x_rel dD''_rho/dx`，`term2 = -[(u_pk-u_bg, v_pk) · grad(D''_rho)]`，`rebuild_W = term1 + term2`。\n');
    fprintf(fid, '- 深度变量：历史字段名 `z_rho_m`、`z_rho_bg_m`、`z_rho_anom_m` 按 `D_rho` 解释，均为正深度向下；默认 MAT/NetCDF 保存原始 `m/s`；PNG 色标显示为 `10^-6 m/s`；白色为空样本格点。\n');
