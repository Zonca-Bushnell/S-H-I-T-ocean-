function write_group_doc(path, matches, grid, polarity, band_label)
    ...
    fprintf(fid, '- 计算边界：composite 阶段只合成等密面几何、速度和支撑；`term1/term2/rebuild_W` 由 `physics/rebuild_w_from_geometry.m` 单独计算。\n');
    fprintf(fid, '- 输出：默认 `matched_core_argo.mat`、`composite_grid.mat`、`composite_grid.nc`、`vertical_transport_terms.png`、`wpk_validation.png`。图像显示为 `10^-6 m/s`，网格文件保存原始 `m/s`，白色为空样本格点。\n');
    fclose(fid);
end
