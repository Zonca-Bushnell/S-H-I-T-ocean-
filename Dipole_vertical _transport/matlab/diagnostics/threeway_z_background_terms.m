function comparison = threeway_z_background_terms(grid3d, polarity, isas_density_mat)
    ...
    panels(1) = make_threeway_panel('A_boa_anomaly', 'A BOA anomaly z''_\rho', ...
        ...
        grid3d.cx_rel .* dzdx_anom, -((grid3d.u_tw - grid3d.mean_cx_raw) .* dzdx_anom + grid3d.v_tw .* dzdy_anom), support_anom, grid3d, []);
    panels(2) = make_threeway_panel('B_absolute_composite_density', 'B composite-density absolute z_\rho', ...
        ...
        grid3d.cx_rel .* dzdx_abs, -((u_abs - grid3d.mean_cx_raw) .* dzdx_abs + v_abs .* dzdy_abs), support_abs, grid3d, panels(1));
    panels(3) = make_threeway_panel('C_boa_background_term2', 'C eddy z_\rho + BOA background term2', ...
        ...
        grid3d.cx_rel .* dzdx_abs, -((u_boa - grid3d.mean_cx_raw) .* dzdx_boa + v_boa .* dzdy_boa), support_abs & support_boa, grid3d, panels(1));
    ...
    if any(isfinite(rho_isas(:)))
        ...
        panels(4) = make_threeway_panel('C_isas_background_term2', 'C eddy z_\rho + ISAS background term2', ...
            ...
            grid3d.cx_rel .* dzdx_abs, -((u_isas - grid3d.mean_cx_raw) .* dzdx_isas + v_isas .* dzdy_isas), support_abs & support_isas, grid3d, panels(1));
    else
