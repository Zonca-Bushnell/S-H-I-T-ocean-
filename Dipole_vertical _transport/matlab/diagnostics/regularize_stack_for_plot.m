function [out, info] = regularize_stack_for_plot(stack, fill_passes, smooth_passes, varargin)
    out = double(stack);
    ...
    for kk = 1:size(stack, 3)
        [out(:,:,kk), one] = regularize_field_for_plot(stack(:,:,kk), fill_passes, smooth_passes, varargin{:});
